...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        jarvis = await e.reply(f"»⚡️𝐌ᴏᴏɴ⚡️_ᴏᴘ_ʙᴏʟᴛ︎")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await jarvis.edit(f"[𝐊𝐈𝐓𝐓𝐘 𝐈𝐒 𝐑𝐄𝐀𝐃𝐘 𝐓𝐎  ](https://t.me/Kittyxupdates)[𝐅𝐔𝐂𝐊 𝐇𝐀𝐓𝐄𝐑𝐒 🥀](https://t.me/Grandxmasti)🤖\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __Moon Ka Ek Beta Aur Add Ho rha hai..__")
        target = ""
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.reply("Only moon view the sudo users list.")
